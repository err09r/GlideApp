...
@Preview
@Composable
fun GraphLatLngPreview(@PreviewParameter(GraphRoutePreviewParameterProvider::class) route: List<Pair<Float, Float>>) {
    GlideAppTheme {
        Graph(
            modifier = Modifier.size(120.dp),
            contentPadding = PaddingValues(4.dp),
...
class GraphRoutePreviewParameterProvider :
    PreviewParameterProvider<List<Pair<Float, Float>>> {

    override val values: Sequence<List<Pair<Float, Float>>>
        get() = sequenceOf(
            listOf(
